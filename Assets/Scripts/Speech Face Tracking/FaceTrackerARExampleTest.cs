...
namespace FaceTrackerExample
{
    ...
    public class FaceTrackerARExampleTest : MonoBehaviour
    {

        bool isAutoResetMode = true;
        public bool drawRect = true;
        public bool drawFace = true;

        public Toggle toggleRect;
        /// <summary>
        ...
        void Start()
        {
            ...
            curr_screenOrientation = old_screenOrientation = Screen.orientation;
        ...
        ScreenOrientation curr_screenOrientation, old_screenOrientation;
        float potrait_cameraSize, landscape_cameraSize;
        bool initOnce, initOnce2;
        public void OnWebCamTextureToMatHelperInitialized()
        {
           
            if (curr_screenOrientation != Screen.orientation)
            {
                curr_screenOrientation = Screen.orientation;
            }
            else
            {
                curr_screenOrientation = old_screenOrientation;
            }
            ...
            if (curr_screenOrientation == ScreenOrientation.Portrait
                && widthScale < heightScale)
            {
                ...
                if (!initOnce)
                {
                    potrait_cameraSize = Camera.main.orthographicSize;                  
                    initOnce = true;
                }                      
            }
            else if (curr_screenOrientation == ScreenOrientation.Landscape
                || curr_screenOrientation == ScreenOrientation.LandscapeRight)
            {
                Camera.main.orthographicSize = height / 2;

                if (!initOnce2)
                {
                    landscape_cameraSize = Camera.main.orthographicSize;                 
                    initOnce2 = true;
                }               
            }
        ...
        IEnumerator AdjustScreenAsIntended(float delay)
        {
            yield return new WaitForSeconds(delay);

            if (initOnce && initOnce2)
            {
                if (curr_screenOrientation == ScreenOrientation.Portrait
                    && Camera.main.orthographicSize == landscape_cameraSize)
                {              
                     Camera.main.orthographicSize = potrait_cameraSize;
                }
                else if ((curr_screenOrientation == ScreenOrientation.Landscape
                    || curr_screenOrientation ==  ScreenOrientation.LandscapeRight)
                    && Camera.main.orthographicSize == potrait_cameraSize)
            ...
            {
                if ((curr_screenOrientation == ScreenOrientation.Landscape
                    || curr_screenOrientation == ScreenOrientation.LandscapeRight))
                {
                    Camera.main.orthographicSize = gameObject.transform.localScale.y / 2;
                }
                else
                {
                    Camera.main.orthographicSize = 
                      (gameObject.transform.localScale.x * (float)Screen.height / (float)Screen.width) / 2;
                }
        ...
        void Update()
        {
            drawRect = toggleRect.isOn;

            StartCoroutine(AdjustScreenAsIntended(0.5f));
